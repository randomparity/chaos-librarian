...
def _rule_sidecar_language_consistent(
    raw: Mapping[str, object],
    line_index: LineIndex,
    collector: IssueCollector,
) -> None:
    ...
    Manifest v3 keys ``ManifestSidecar`` lookups on ``(asset_id, language)``.
    Two failure modes break that invariant:

    1. Duplicate ``(target, language)`` across ``create_sidecar`` events
       in the same scenario would produce two sidecar rows with the same
       composite key, leaving the key ambiguous.
    2. ``language`` that doesn't appear in the target asset's declared
       ``subtitles[*].language`` is a typo or an oversight — the
       materializer hashes subtitles from ``asset.subtitles`` by declared
       language, so a mismatched event-language would either leave the
       plan-created row hashless or append a second row for the same
       on-disk file.
    """
    # Build a lookup of declared subtitle languages per asset.
    declared_by_asset: dict[str, set[str]] = {}
    for asset, _loc in _iter_assets_with_loc(raw):
...
_RULES: list[_Rule] = [
    ...
    _rule_sidecar_language_consistent,
]
