...
_RESOLUTION_SWITCH_REJECTED_VIDEO_FIELDS: Final[tuple[str, ...]] = (
    "vfr_cadence",
    "field_order",
    "color_space",
    "color_range",
...
_CONSTRAINED_VIDEO_REJECTED_ASSET_LISTS: Final[tuple[tuple[str, str], ...]] = (
    ("audio", "audio streams"),
...
def _check_resolution_switch_video(
    *,
    ...
    _reject_matroska_muxing_profile_for_asset(
        ...
        asset_loc=asset_loc,
        reporter=reporter,
        reason=f"cannot be combined with {mode_label}",
    )
    ...
    _reject_video_mode_incompatibilities(
        asset=asset,
        video=video,
        asset_loc=asset_loc,
        video_loc=video_loc,
        reporter=reporter,
        combination_label=mode_label,
        rejected_video_fields=_RESOLUTION_SWITCH_REJECTED_VIDEO_FIELDS,
    )
...
def _reject_video_mode_incompatibilities(
    *,
    asset: Mapping[str, object],
    video: Mapping[str, object],
    asset_loc: _Loc,
    ...
    for field_name, label in _CONSTRAINED_VIDEO_REJECTED_ASSET_LISTS:
        if not _as_list(asset.get(field_name)):
            continue
        reporter.error(
            code=E_MATERIALIZE_UNSUPPORTED,
            message=f"{combination_label} does not support {label}",
    ...
    _reject_embedded_metadata_for_asset(
        ...
        asset_loc=asset_loc,
        reporter=reporter,
        reason=f"cannot be combined with {combination_label}",
    )
    for field_name in rejected_video_fields:
        if field_name not in video or video.get(field_name) is None:
            continue
        reporter.error(
            code=E_MATERIALIZE_UNSUPPORTED,
            message=f"{combination_label} cannot combine {field_name}",
            loc=(*video_loc, field_name),
        )
...
def _check_mode_expected_string_field(
    ...
    *,
    ...
    if not isinstance(value, str) or value == expected:
        return
    if message is None:
        message = f"{mode_label} requires {field_name} {expected!r}"
    reporter.error(
        code=E_MATERIALIZE_UNSUPPORTED,
        message=message,
        loc=loc,
