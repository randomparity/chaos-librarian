...
class TestRuleSidecarLanguageConsistent:
    """create_sidecar (target, language) must match a declared subtitle and be unique.

    WHY: manifest v3 keys ``ManifestSidecar`` lookups on ``(asset_id, language)``.
    A duplicate pair leaves the key ambiguous; an undeclared language
    diverges from the asset.subtitles[*].language list that the
    materializer hashes from.
    """

    def test_undeclared_language_flagged(
            ...
            timeline=[
                {
                    ...
                    "target": "a",
                    "to": "r/a.fra.srt",
                    "language": "fra",  # not declared on asset 'a' (only "eng")
                },
            ],
        )
        # Replace the minimal asset with one that declares an "eng" subtitle.
        ...
        assert any("not declared" in i.message and "fra" in i.message for i in issues)
