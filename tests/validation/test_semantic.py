...
class TestRuleLifecyclePostDeleteMutations:
    ...
    def test_reencode_video_after_delete_emits(self) -> None:
        raw = _minimal_scenario(
            timeline=[
                {"id": "e1", "at": "0", "action": "delete_file", "target": "a"},
                {
    ...
    def test_reencode_audio_after_delete_emits(self) -> None:
        raw = _minimal_scenario(
            timeline=[
                {"id": "e1", "at": "0", "action": "delete_file", "target": "a"},
                {
    ...
    def test_create_sidecar_after_delete_emits(self) -> None:
        raw = _minimal_scenario(
            timeline=[
                {"id": "e1", "at": "0", "action": "delete_file", "target": "a"},
                {
...
class TestRuleLifecycleDuringPendingSlowCopy:
    ...
    def test_delete_during_slow_copy_emits(self) -> None:
        raw = _minimal_scenario(
            timeline=[
    ...
    def test_move_during_slow_copy_emits(self) -> None:
        raw = _minimal_scenario(
            timeline=[
                self._slow_copy_start(),
                {
    ...
    def test_rename_during_slow_copy_emits(self) -> None:
        raw = _minimal_scenario(
            timeline=[
                self._slow_copy_start(),
                {
    ...
    def test_back_to_back_starts_after_commit_allowed(self) -> None:
...
class TestRuleAssetPathSafety:
    ...
    def test_asset_id_with_traversal_flagged(self) -> None:
        raw = _minimal_scenario()
        assets = _as_list(
            _as_dict(_as_list(_as_list(raw["works"])[0]["variants"])[0]["bundle"])["assets"]
        )
        _as_dict(assets[0])["id"] = "../../escape"
        collector = IssueCollector()
        run_semantic_pass(raw, _empty_index(), collector)
    ...
    def test_asset_container_with_separator_flagged(self) -> None:
        raw = _minimal_scenario()
        assets = _as_list(
            _as_dict(_as_list(_as_list(raw["works"])[0]["variants"])[0]["bundle"])["assets"]
        )
        _as_dict(assets[0])["container"] = "../foo"
        collector = IssueCollector()
        run_semantic_pass(raw, _empty_index(), collector)
        issues = [i for i in collector.issues if i.code == codes.E_PATH_CONTAINMENT]
        assert any("asset.container" in i.message and "../foo" in i.message for i in issues)

    def test_safe_asset_passes(self) -> None:
        raw = _minimal_scenario()
        collector = IssueCollector()
        run_semantic_pass(raw, _empty_index(), collector)
