...
- A completed wall-clock journal matches the planned journal prefix identified by
  `replay.json.applied_events` after stripping `wall_clock_time`.
...
chaos-librarian run scenario.yaml --out fixtures/run-001 --duration 90s --speed 10x
  cli.commands.run
    ...
    allocate materialized run_id
    preflight_timeline + capability checks          # exit 4/5 on unsupported tools/actions
    run_plan(..., run_id=run_id, steps_limit=None)  # full logical schedule + journal
    ...
    while elapsed < requested_duration or event/slow-copy pair in progress:
      logical_now_ns = floor(elapsed_wall_ns * speed_multiplier)
      for each due journal entry in order:
        re-check monotonic deadline before starting it
        stop if deadline passed, unless this entry commits an in-flight slow-copy
        execute the entry
        append the executed entry to journal.jsonl
...
Wall-clock journal entries are generated by the same logical event application
as plan/step, but with the materialized `run_id` allocated for this `run` before
journal construction. The scheduler fills `wall_clock_time` at execution time
and must not change any other logical field. Comparisons generate the planned
prefix with the same materialized `run_id`, then strip `wall_clock_time`; they do
not compare against a plan-only journal carrying the deterministic plan-only
`run_id`.
...
- Parsed as exact decimal/rational arithmetic, never binary float.
- `logical_now_ns` is `floor(elapsed_wall_ns * speed_multiplier)`.
...
`speed_multiplier` is serialized as the normalized multiplier string without
the trailing `x`: `0.50x` becomes `"0.5"`, `1.0x` becomes `"1"`, and `10x`
becomes `"10"`.
...
Wall-clock `run` preflight enforces this before run-dir publication: a
`slow_copy_start` whose immediate next resolved event is not its paired
`slow_copy_commit`, or a `slow_copy_commit` without an immediately preceding
paired start, raises `TimelineUnsupportedError`
(`E_MATERIALIZE_TIMELINE_UNSUPPORTED`) and exits 5. This is a run-mode
unsupported-timeline failure, not scenario validation; `plan`, `step`, and
batch `materialize` semantics remain unchanged.
...
- `execution_mode: "run"`
- random materialized `run_id`, shared by `journal.jsonl`, `replay.json`,
  sentinel, and `materialization.json`
- `created_at`
- `toolchain`
- `applied_events` equal to the executed raw journal-entry count
- `journal_digest` over the emitted executed logical journal bytes after
  omitting `wall_clock_time`
...
- Recompute the logical journal prefix with the recorded resolved seed and
  recorded materialized `run_id`.
...
- Finish the handler.
- Re-check the monotonic deadline before starting another journal entry.
- Start no further due event after the deadline, except the paired
  `slow_copy_commit` needed to close an already-started slow-copy window.
- Finalize after the current handler, or that required commit, completes.
...
- Wall-clock-only unsupported timeline shapes, including non-adjacent
  slow-copy pairs, use `TimelineUnsupportedError`
  (`E_MATERIALIZE_TIMELINE_UNSUPPORTED`) and exit 5 before run-dir publication.
...
- due-event calculation maps wall elapsed to logical time using exact floor
  arithmetic
- fractional speed boundary cases do not use float rounding
- timeline-drained runs idle until duration
- in-flight event overrun finalizes after handler completion
- a handler overrun does not start a second due event after the deadline
...
- journal, replay, sentinel, materialization report, and digest all use the same
  materialized `run_id`
- run preflight rejects non-adjacent slow-copy pairs before publication
...
- non-adjacent slow-copy pairs exit 5 with
  `E_MATERIALIZE_TIMELINE_UNSUPPORTED` and no published run dir
...
- partial-duration wall-clock journal matches the planned journal prefix
  identified by `replay.json.applied_events` after stripping `wall_clock_time`
- full-duration wall-clock journal matches the full plan/step logical journal
  after stripping `wall_clock_time`
...
materialize, and run drift likely. Sprint 8's key contract is that completed
wall-clock journals remain logically identical to the planned journal prefix
identified by `replay.json.applied_events`.
